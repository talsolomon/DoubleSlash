...
description: The calling operator (Dora / Sol / Bran / May) produces a validated <fish-handoff> block for a forward phase transition. Schema-validated against fish-model.md §9. Refuses to emit if the archetype's required methods aren't run or explicitly deferred. Writes to disk, appends to card history, displays inline. The universal state-passing mechanism between phases.
---
...
- **A validated `<fish-handoff>` block** — rendered to the canonical schema from [`methodology/fish-model.md §9`](../../../methodology/fish-model.md#9-the-flow-handoff-block). Certainty-first sigil, archetype, phase_exited, methods_run, locked, open, artifacts, decisions, confidence_to_advance, notes.
...
| Check sigil-method coverage against the phase × archetype cell from fish-model.md §6 | Decide: run the missing methods, or defer each with an explicit reason in `notes` | If you defer, the reason lands in notes so the next operator can see what was skipped and why |
...
- **Missing required methods** for the archetype × phase cell — without a deferral reason in `notes`. This is the fish-model.md §6 matrix being prescriptive; HO is where it gets enforced.
...
HO is the **exit gate for every forward phase transition** — Explore → Solidify, Solidify → Build, Build → Ship, Ship → Explore (next loop). Most operator workflows end with an HO call. If you're writing an operator workflow that doesn't end with HO somewhere, it's probably mid-phase and should end with a Question, Review, or Block instead (fish-model.md §10 — the four agent moves).
