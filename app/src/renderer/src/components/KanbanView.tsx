...
export default function KanbanView({
  spaces, activeContextId, selectedProjectId, onSelect, onSetActive, onUpdate, onRefresh,
}: Props) {
  const [recipeOpen, setRecipeOpen] = useState(false)
  const [drilldownContextId, setDrilldownContextId] = useState<string | null>(null)
  ...
  const drilldownContext = drilldownContextId
    ? contexts.find(c => c.id === drilldownContextId) ?? null
    : null
    ...
    <div className="h-full flex flex-col relative overflow-hidden">
        ...
        <div className="flex flex-1 overflow-x-auto">
          {PHASES.map(phase => (
            <PhaseColumn
              ...
              filter={filter}
              width={COL_WIDTH}
              onOpen={id => { setDrilldownContextId(id); onSelect(id) }}
      ...
      {drilldownContext && (
        <DrillDownDrawer
          ...
          onClose={() => setDrilldownContextId(null)}
          onUpdate={onUpdate}
          onRefresh={onRefresh}
        />
      )}
