...
**As a** developer on the Duble Slash team,
...
**As a** Senior IC using Duble Slash,
**I want** a persistent macOS menubar icon that shows capture state at a glance and opens a quick-info popover,
**So that** I can see Duble Slash is working and take quick actions (pause, open) without context-switching.
...
- [ ] AC-004: Clicking the icon opens a 280pt-wide popover showing: one-line status, today's session count per tool, [Pause capture] button, [Open Duble Slash] button.
...
**As a** user installing Duble Slash for the first time,
**I want** a guided, under-5-minute onboarding that gets me from install to first capture,
**So that** I feel in control of what Duble Slash does, and I end up with at least one captured session.
...
**As a** user granting Duble Slash Accessibility permission (and as any user navigating the app),
...
**So that** I understand what I'm granting, and I can use Duble Slash without a mouse if I need to.

**Acceptance Criteria:**
- [ ] AC-001: Before the macOS Accessibility dialog opens, Duble Slash shows a panel explaining exactly what will be read and why, with a single [Open System Settings] action.
- [ ] AC-002: Clicking [Open System Settings] deep-links to the correct pane in System Settings → Privacy & Security → Accessibility.
- [ ] AC-003: If the user denies or dismisses the dialog, Duble Slash gracefully disables Tier 3 and surfaces a non-blocking message per REQ-005.
...
The core reason Duble Slash exists. Ship three independent capture paths that together cover every AI tool the beachhead persona uses daily. Each tier is independent — any one failing must not break the others.
...
**I want** to paste my keys into Settings and have Duble Slash store them securely,
...
- [ ] AC-004: Screen shows persistent reassurance copy: *"Duble Slash never sees these keys leave this device."*
...
**I want** an extension that captures my sessions and sends them to the Duble Slash desktop client,
...
- [ ] AC-003: Extension icon shows a "Duble Slash is capturing" badge when active on a supported site.
- [ ] AC-004: Extension never transmits captured content to any endpoint other than the local Duble Slash desktop client.
...
- [ ] AC-006: If the buffer fills (>10 sessions pending), the extension notifies the user in the extension popup with an actionable message ("Duble Slash desktop client unreachable — open it to catch up").
...
**I want** Duble Slash to capture my sessions from those native apps,
...
**So that** I never feel like Duble Slash has my data hostage.
...
Two in-process agents do the work that makes Duble Slash feel alive: tag every session with useful metadata, and flag anything sensitive before it could ever be pushed. Every agent action is logged and narratable.

### Story 4.1 — Agent runtime + audit log

**As a** user of Duble Slash,
...
**I want** Duble Slash to detect sensitive content in every session and show me what *would* be redacted if pushed,
...
- [ ] AC-006: Detection rules live in `planning/redaction-rules.md` (new file) so future contributors can propose patterns via PR.
...
**As a** Senior IC opening Duble Slash after a morning of AI work,
...
- [ ] AC-005: Empty state (no sessions ever) shows the v1.1 reframe copy: *"Duble Slash is watching {N} tools. Start a session in {Claude/Cursor/ChatGPT} and it'll show up here — ready to hand off to a teammate when V1 ships."* Footer: *"Nothing has left this device."*
...
**I want** a single page that answers "what does Duble Slash know and what has left this device?",
**So that** I can screenshot it and recommend Duble Slash to my team without equivocation.
...
**So that** I can configure Duble Slash and trust the redaction model.
...
**As a** Duble Slash team member measuring adoption,
...
- [ ] AC-003: Destination is a self-hosted PostHog OSS instance under the Duble Slash team's control.
...
**As a** Duble Slash team member fixing crashes,
...
- [ ] AC-003: Destination is a self-hosted Sentry OSS instance under the Duble Slash team's control.
...
The non-code work that makes Duble Slash a credible OSS launch. If this epic slips, the code could ship but the *launch* can't.

### Story 7.1 — Public GitHub repo + FSL-1.1-Apache-2.0 license + CLA

**As a** prospective contributor,
**I want** to land on the Duble Slash repo and immediately understand the license, contribution path, and governance,
**So that** I don't waste my time or fear being sued.

**Acceptance Criteria:**
- [ ] AC-001: Two public repos exist in the Duble Slash GitHub org: one for the client+extension (FSL-1.1-Apache-2.0), one for the capture protocol (Apache 2.0).
...
**As a** security-conscious user auditing Duble Slash's supply chain,
**I want** every release to be signed and accompanied by an SBOM with no license landmines,
**So that** I can verify provenance and recommend Duble Slash to my team with confidence.
...
### Story 7.3 — Marketing site at `dubleslash.<domain>`

**As a** visitor arriving from Hacker News or Twitter,
**I want** a fast, honest landing page that explains what Duble Slash does and lets me install it in under a minute,
**So that** my first impression is calm confidence, not marketing noise.

**Acceptance Criteria:**
- [ ] AC-001: Site at `dubleslash.<domain>` (exact domain TBD) ships with: hero + tagline + 20s demo loop (menubar + Today view), Privacy-dashboard screenshot as the centerpiece, "how it works" tier diagram, founder story (Tal + Shenhav), install + GitHub + license links.
- [ ] AC-002: Site does NOT include testimonials, enterprise logos, or customer quotes.
- [ ] AC-003: Site hero states the team-collab framing from v1.1: Duble Slash is a collaboration tool for design+dev teams, not a personal AI brain.
- [ ] AC-004: Site is built on Next.js + Vercel and open-sourced in the Duble Slash GitHub org under the same FSL license.
...
5. Draft `planning/redaction-rules.md` as called out in Story 4.3 AC-006.
