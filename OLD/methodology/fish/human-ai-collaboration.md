---
name: Fish Model × human-AI collaboration — the moat
description: How Fish Model turns AI tools from eager-but-directionless assistants into phase-shaped collaborators. The specific value Duble Slash gives designers who have already started working with AI.
type: methodology-spec
---

# Fish Model × human-AI collaboration — *the moat*
...
2. What the Fish Model specifically adds.
3. What Duble Slash specifically layers on top of Fish Model.
4. The design principles every Fish Model agent obeys to protect the human.
...
**What the Fish Model does:** the Explorer agent refuses Solidify-shaped requests. It will *name the phase the request belongs to* and offer to hand off — rather than silently drift.
...
**What the Fish Model does:** the archetype × phase matrix tells the agent what *right-sized* looks like for this card. A Nemo's Explore is a 10-minute heuristic scan, not a thesis. The Explorer refuses to generate a thesis for a tooltip.
...
**What the Fish Model does:** every phase exit emits a `<fish-handoff>` block — short, copy-pasteable, machine-readable. It captures the sigil, what's `locked`, what's `open`, which artifacts exist, and the outgoing agent's honest confidence in advancing. Paste it into tomorrow's session and the Solidifier picks up exactly where Explorer left off. No re-briefing.
...
**What the Fish Model does:** the Shipper emits a **trust receipt** on every release — a short, signed summary of *what shipped, what was redacted, who approved it*. Even for solo work. Trust receipts are auditable, pasteable, and eventually signable by both the human and (in V1) a teammate.

---

## 2. What Fish Model adds — six concrete shifts

| Without Fish Model | With Fish Model |
|---|---|
...
None of these require a new tool. They require a **method the AI can read and enforce**. Fish Model is that method.

---

## 3. Where Duble Slash specifically wins

Fish Model is an open methodology — anyone can adopt it. **Duble Slash's competitive moat** is that we make Fish Model operable *across tools, across sessions, and across teammates* without making you switch tools.
...
| **Capture agent** (OSS launch) | Every Fish Model session is recorded with its sigil, phase, and handoffs — so your "yesterday" is never lost, even if you closed the tab. |
| **Redaction agent** (OSS launch) | Handoffs and trust receipts strip secrets and PII before they ever leave your laptop. Fish Model's auditability doesn't require sacrificing privacy. |
...
| **Handoff agent** (V1) | A teammate's Solidifier picks up *your* Explorer handoff. The Sarah→Marcus demo is literally Fish Model's handoff contract, multiplayer. |
...
**The compounding move:** Fish Model alone is useful to a solo designer. Fish Model + Context Cloud is useful across their tools. Fish Model + Context Cloud + multiplayer handoff is useful to their team. Fish Model + all of the above + `flow.yaml` is the foundation other tools (Linear, GitHub, Figma) integrate *with*. Each layer is voluntary; each adds moat.
...
- **With Duble Slash + Fish Model:** one card per problem, sigil stamped, four handoffs, full thread recoverable — still inside Claude. No new tool to learn, no Notion-filling ritual.

### Design lead
- **Before:** You don't know what your team is doing inside their AI tools. "How's the onboarding redesign?" → vague answers.
- **With Duble Slash + Fish Model:** your team's cards carry sigils and phases. The Digest agent (V1.5) shows "3 Willies in Solidify, 2 Nemos ready to Ship." You see the *shape* of the team's work without reading every chat.
...
- **With Duble Slash + Fish Model:** the Solidifier's `locked` list *is* the acceptance criteria. The Builder consumes it directly. Re-interpretation shrinks to translation.

### New joiner
- **Before:** two weeks of Slack scroll + "ask the old-timer."
- **With Duble Slash + Fish Model:** Context Bundler (V1) hands them a pack of recent Tuna and Willie handoffs, archetype-tagged. They see the shape of the product's in-flight work, not a wall of messages.

---

## 5. Design principles every Fish Model agent obeys
...
### With Fish Model (solo, OSS-launch capabilities)
...
## 7. What Fish Model is *not* doing for you

To be honest about the boundary:

- **Fish Model doesn't replace design skill.** A bad Solidify brief from a skilled designer is still better than a well-formatted Solidify brief from someone who doesn't know the user.
- **Fish Model doesn't guarantee a good outcome.** It guarantees a *legible process*. Some cards ship bad things cleanly.
- **Fish Model doesn't work if you skip the handoff.** The whole chain depends on it. An undocumented phase transition is a hole in the method.
- **Fish Model alone is not the moat.** Fish Model + `//` + Context Cloud + multiplayer handoff is the moat. A designer can get most of Fish Model's value from a well-written CLAUDE.md; we just make the rest effortless.
...
## 8. Agent state model — what every Fish Model agent tracks

Sections 1–7 made the case. Sections 8–10 make the case **operable**: if Fish Model is a contract the AI can finally read, this is where the contract gets signed.

Every Fish Model agent — local or system, operator (Dora/Sol/Bran/May) or expert (Design/PM/Dev/…) — maintains the same minimum state. Interoperability depends on this: Sol can pick up what Dora wrote because they speak the same state; a teammate's agent tomorrow can pick up yours because the state is in the handoff, not in the agent's head.
