...
**Who's at the wheel.** Dora. Classifier mode — her job here isn't to be clever, it's to get the sigil honest. *"You don't get to call a Salmon a Tuna because the deadline is Tuesday"* (fish-model.md §4.3).
...
- **The Explore cell read-out** — the specific methods fish-model.md §6 expects for this archetype's Explore phase. You know what's coming.
...
| Derive the archetype mechanically from (certainty, size) | Confirm the archetype or push back with reasoning | If you push back without reasoning, Dora quotes fish-model.md §4.3 — sigil is mechanical, deadline pressure is not a valid re-sigil reason |
...
**Decision order matters.** Dora asks *certainty before size*. Size estimates are unreliable in a space the team doesn't understand, so checking familiarity first anchors the honest-size conversation (fish-model.md §3.1).
...
Sigil is also **mutable** — if HS or IP reveals the card is actually bigger or less familiar than you called it, Dora narrates the re-sigil and the matrix cell updates (fish-model.md §3.4 + §11.2). Running SA again mid-card is normal.
