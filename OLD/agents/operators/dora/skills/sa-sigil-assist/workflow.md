...
**Decision order:** certainty first, then size. Size estimates are unreliable in an unfamiliar space, so you check what the team knows before guessing surface area (fish-model.md §3.1).

Source: [`methodology/fish-model.md §3`](../../../../../methodology/fish-model.md#3-the-sigil) (sigil axes + defaults) · [`§4`](../../../../../methodology/fish-model.md#4-the-four-archetypes) (archetype derivation).
...
<step n="2" goal="Probe certainty axis FIRST — prior familiarity, NOT technical complexity">
  <critical>Certainty is measured by the TEAM's prior familiarity with the user and solution space (fish-model.md §3.1). A billing flow is Known for a payments team, Unknown for a greenfield startup.</critical>
  <critical>Certainty is decided BEFORE size. Size estimates are unreliable in a space the team doesn't understand, so we check familiarity first (fish-model.md §3.1).</critical>
...
<step n="3" goal="Probe size axis — scope surface area, NOT effort">
  <critical>Size is measured by expected surface area of the final artifact, not effort or duration (fish-model.md §3.2).</critical>
...
Dora: "Quick size check — three probes:
  ...
  <check if="user is genuinely uncertain OR certainty=unknown">
    <action>Apply the default rule from fish-model.md §3.2: default to `bigger`. Narrate: "When certainty is unknown or the size is hard to call, I default to bigger — over-sizing costs a bit of overhead; under-sizing skips research and ships broken work."</action>
  </check>
</step>

<step n="4" goal="Derive archetype mechanically">
  <action>Map (certainty, size) to archetype per fish-model.md §4:</action>
...
<step n="5" goal="Confirm or override">
  <output>
Dora: "Want me to proceed on this sigil, or does something feel off? (fish-model.md §4.3 — sigil is mechanical, but honest re-reads are fine.)"
...
<step n="7" goal="Recommend next method">
  <action>Propose the next Explore method based on archetype (per fish-model.md §7.1 streams):</action>
...
<facilitation-guidelines>
  <guideline>SA is mechanical, not political. If the user argues "we have to call this a Nemo because the deadline is Tuesday," quote fish-model.md §4.3 directly: "you don't get to call a Salmon a Tuna because the deadline is Tuesday."</guideline>
  <guideline>Sigil is mutable (fish-model.md §3.4). If probes reveal Willy-scale complexity after an initial Nemo call, narrate the change and extend Explore — don't smuggle.</guideline>
  <guideline>Don't skip the Hebrew names. Tal uses them intentionally; they're part of Fish Model's voice.</guideline>
</facilitation-guidelines>
