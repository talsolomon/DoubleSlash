---
name: Fish Model — the method (one doc)
description: The canonical Fish Model spec — the AI-era evolution of the 2024 Fish Model. One file, every concept. Axes, fish types, phases, curated method sets per cell, per-fish streams, handoff contract, agent posture, worked examples.
type: methodology-spec
owner: Tal
version: 2.0.2
renamed_from: FLOW (was FISH before that)
revised: 2026-05-03 (v2.0.2: renamed FLOW → Fish Model; flow.md → fish-model.md)
lineage: Bánáthy (1960s–70s) → Double Diamond (UK Design Council, 2004/2019) → Fish Model (Tal Solomon, 2024) → Fish Model v2 (this spec, 2026)
---
...
**Fish Model** is how you run one unit of design / product / engineering work — from messy problem to shipped artifact — so that humans, AI agents, and teammates all describe the same work the same way, at the right intensity, with clean handoffs between phases.
...
- **Origin B — AI-era, 2026 →** **Fish Model** — the Fish Model in motion: iteration first-class, AI agents nudging the method forward, machine-readable handoffs that travel across sessions / tools / teammates. The fish was a static diagram; Fish Model is the fish moving. Agents make discipline cheap — humans-alone often skip their own method; agents don't.
...
**The work-classification noun is still "fish"** — you ask *"what fish is this?"* and get Nemo / Tuna / Salmon / Willy. **The methodology is the Fish Model** — the discipline that runs around the fish. (Pairs with the company value *"Flows, not frames."*)

---

## Table of contents

1. [Why Fish Model exists](#1-why-flow-exists)
...
9. [The `<fish-handoff>` block](#9-the-flow-handoff-block)
...
15. [What Fish Model is *not*, and lineage](#15-what-flow-is-not-and-lineage)

---

## 1. Why Fish Model exists
...
**[AI-era]** In 2026, AI co-workers broke something else — they broke the designer's *orientation*. Users report a specific cluster of pains: *"I don't know where I am. I don't trust any of this is saved. The AI doesn't guide me. It has no UX instinct. Switching tools means starting over. I can't see the project — just pieces. There's no history of who decided what."* (Seven real pains, each mapped to a Fish Model fix, in §13.)

The Fish Model in 2026 is the fix: the same Fish Model, now **operable by AI agents** that nudge the method forward, *and* **portable across tools / sessions / teammates** so the project stays consolidated.

**Positioning:** Fish Model is **Agile for the AI era** — **user-centered**, design-thinking-shaped, AI-native. Double Diamond is the backbone because *the user* is the point; the project's success is the signal that the user won. It replaces sprint cadence, PRDs, and standups with a continuously-shippable per-card lifecycle that the AI can run alongside you without taking over. See §15 for what it replaces, what it is, and what it is not.
...
> **Note (AI-era):** the original 2024 Fish Model used sprint-band estimates (≤1 sprint, 1+, 2+, 3+). With AI agents running method discipline continuously, The Fish Model doesn't assume sprint cadence anymore — work ships when the phase exits cleanly, not when a sprint clock rolls over. Sprint bands retained as rough calibration only; not a schedule contract.
...
Classical Double Diamond has two diamonds: *problem space* and *solution space*. The Fish Model splits each into its own phase — with its own agent, artifact, and handoff contract — so that:
...
- **Inside each phase** — micro-loops (ask → answer → sharpen the question) that AI makes fast. A Salmon Explore might run the clustering loop five times in a day. The Fish Model makes each loop visible without demanding a phase transition.
...
The linear arrow Explore → Solidify → Build → Ship is the *default path*, not a cage. The Fish Model's job is to make loops and reverses **legible**, not to prevent them.
...
- **Artifacts:** raw notes, interview transcripts, journey maps, competitor screenshots, HMW reframes, nugget board, one-paragraph framing summary, `<fish-handoff>` to Solidifier.
...
- **Entry:** `<fish-handoff>` from Explorer with a framing, unknowns list, and artifacts (or — for Nemo / Tuna — a one-sentence brief if entering Solidify directly).
...
- **Entry:** `<fish-handoff>` from Solidifier with shape locked, AC written, `open` empty of blockers. For Salmon / Willy: a measurement plan.
...
> **Naming note.** The Hebrew name for this phase is עיצוב ("design") — from the original Fish Model's design-led framing. The Fish Model renames it **Build** because in the AI-era this phase includes engineering execution, not only design-mode prototyping. The method set here spans both (see §6).
...
- **Entry:** `<fish-handoff>` from Builder with runnable / reviewable artifact. Any measurement plan from Solidify is implemented.
...
> **Naming note.** Hebrew מימוש ("realize") is release-plus-validation. The Fish Model shortens to **Ship** to emphasize the release act; validation happens in-Ship via instrumentation, not as a separate phase.
...
5. Roll out staged — internal → alpha → beta → general. Each stage emits its own `<fish-handoff>` (intra-Build).
6. Measure per slice.
...
Every method a Fish Model agent may reference. Grouped by purpose. Each carries phase tag(s) and source / lineage where relevant.
...
- **`<fish-handoff>`** — universal transition block (§9). Emitted at every phase exit.
...
In V1, `<fish-handoff>` gains three optional fields:
...
**[AI-era]** Phase transitions aren't the only moments an agent needs to signal upward. Entry-condition failures, scope changes, and mid-phase concerns all require a move. Fish Model uses four typed moves — same vocabulary across all agents — so the human reading chat can tell at a glance whether work can proceed:
...
| **Handoff** | Emit `<fish-handoff>` to the next-phase agent. | No — normal transition. | Builder → Shipper at Build exit. |
...
- **Silently do the cross-phase work and hope you don't notice.** Silent drift is the bug Fish Model exists to prevent.
...
**[AI-era]** The original 2024 Fish Model assumed humans running the method with their own discipline. AI changed the conversation, and not always for the better. What follows are the *actual* pains designers, PMs, and devs report when building product with AI in 2026 — and how Fish Model addresses each.

### 13.1 Seven pains, seven fixes

| What users feel | What's actually happening | How Fish Model fixes it |
|---|---|---|
...
These are the reasons AI-assisted projects drift, ship late, or quietly die. Fish Model addresses each one mechanically — not through good intentions.

### 13.2 None of this requires a new tool

The Fish Model doesn't want you to leave Claude / Cursor / ChatGPT. It wants your AI to *finally behave like a methodology knows what it's doing*. The method is a shared vocabulary the AI can read and run. The artifact is a card + handoffs that survives sessions and tools. That's it.
...
See [agents-overview.md](./agents-overview.md) for the full 13-agent roster, including the 9 Layer-3 system agents (Duble Slash Cloud) that extend Fish Model across sessions and teammates.
...
**What Fish Model did:** 2 hours total, no scope creep. Explorer refused to wireframe a dropdown; Solidifier refused to spin a dropdown-redesign into this card. Zero PRD. Zero journey meeting. Four handoffs, each 30 seconds. Card is resumable if interrupted.
...
**What Fish Model did:** Brief replaced PRD. Storyboard was the meeting material; AC was the spec; trust receipt was the QA artifact. No translation layer between design and engineering. Reusable journey map is now the foundation for the next billing Tuna.
...
**What Fish Model did:** Willy-sized Explore protected against the "Sarah → Marcus seems simple" trap — the premortem's 3 failure modes became design inputs, not launch bugs. Break-into-Tunas kept the Willy build-able. Staged Ship meant measurement didn't wait for GA. Decision log became the onboarding doc for the next Willy — institutional memory instead of archaeology.

---

## 15. What Fish Model is, what it replaces, and lineage

### 15.1 What Fish Model is

**The Fish Model is Agile for the AI era.** A **user-centered**, design-thinking-shaped methodology built on the assumption that AI is present and gets a *role*. The output is a project that **ships** — with faster iteration, consolidated context, reviewable decisions, and human judgement preserved.

**User-centered, not KPI-driven.** Double Diamond is the backbone because the user is the point. A project's "success" is the signal that the user won — not a metric detached from the user's life. Fish Model exists to make user-centered design-thinking survive AI's arrival, not to replace it with outcome-math.

### 15.2 What Fish Model replaces
...
### 15.3 What Fish Model is *not*

- **Not project-management software.** Fish Model runs *one project's lifecycle*; portfolio view sits above.
- **Not design-only.** Phases and handoffs apply to code, research, docs, decisions.
- **Not tool-prescriptive.** Figma, VS Code, notebook, napkin — Fish Model cares about phase + fish + handoff, not filetype.
...
- **Not a guarantee of good work.** A shallow Explore from a skilled designer still beats a thorough Explore from someone who doesn't know the user. Fish Model shapes the process; the human supplies the taste.
...
**Fish Model (this spec, 2026)** is the AI-operable evolution: same lineage, newly legible to LLMs and to a multiplayer graph. Specifically, Fish Model adds:
...
- `<fish-handoff>` as the universal, plain-text transition receipt.
...
The operators and experts that enact Fish Model are specified in:
