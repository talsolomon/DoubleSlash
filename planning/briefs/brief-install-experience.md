...
2. **`AGENTS.md` merge strategy** when the user's repo already has one — append? Section-stamp? Refuse? Current direction: append under a `## FLOW` heading with an idempotent marker so re-installs are no-ops.
