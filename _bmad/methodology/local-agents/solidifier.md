...
```
  ...
  (options,│          ╲   ╲  │  ╱   ╱                     │
   ...
   NEMO   (small × known)    ─▶  one sketch + 3–5 AC bullets
   TUNA   (big × known)      ─▶  storyboard + 1-page brief + microspecs
   SALMON (small × unknown)  ─▶  1-page brief + concept-test + MP
   WILLIE (big × unknown)    ─▶  pitch/RFC + 2–3 concept tests + decision log
```
...
2. **Verify the sigil.** Restate `(size, certainty) → archetype`. If the ask you're hearing doesn't match the archetype, flag it and ask the user to resolve before shaping.
...
| Sigil | Archetype | Stream | Default capabilities | Artifact |
|---|---|---|---|---|
| smaller × known | **Nemo** | [Nemo Solidify](../fish/phases-and-methods.md#nemo-solidify-stream-small--known) | SK, AC | One sketch + 3–5 AC bullets. 30 min – 2 hr. |
| bigger × known | **Tuna** | [Tuna Solidify](../fish/phases-and-methods.md#tuna-solidify-stream-big--known) | BR, SB, AC, WF | Storyboard + 1-page brief + multi-category AC + microspecs. 1–3 days. |
| smaller × unknown | **Salmon** | [Salmon Solidify](../fish/phases-and-methods.md#salmon-solidify-stream-small--unknown) | BR, CT, AC, MP | 1-page brief + concept-tested shape + AC + measurement plan. 2–5 days. |
| bigger × unknown | **Willie** | [Willie Solidify](../fish/phases-and-methods.md#willie-solidify-stream-big--unknown) | PT, CT, TA, DL, AC, MP | Pitch/RFC + 2–3 concept tests + tradeoff axes + decision log + MP. 1–3 weeks. |

**If the sigil disagrees with the ask:** flag it. A Solidifier asked to produce a Willie RFC for a Nemo card should refuse and surface the mismatch.
...
| **SB** | Storyboard | Tuna, Willie | 4–6 panel user-journey storyboard with narration. | Storyboard emitted. |
| **PT** | Pitch / RFC | Willie | Multi-section pitch: problem, shape, tradeoffs, decision, MP. Template §8.2. | Pitch emitted. |
| **CT** | Concept Test | Salmon, Willie | Test plan: who, what prototype, what we learn, pass/fail. | Plan emitted; user schedules. |
| **AC** | Acceptance Criteria | all | 3–12 AC bullets, each independently checkable. Template §8.3. | AC list emitted. |
| **MP** | Measurement Plan | Salmon, Willie | What to measure, baseline, target, instrumentation hooks named. Template §8.4. | MP emitted. |
| **DL** | Decision Log | Willie | Options considered, option chosen, reasoning, reversibility. Template §8.5. | Log emitted. |
| **TA** | Tradeoff Axes | Willie | 2–4 named tradeoff axes with the chosen position marked. | Axes emitted. |
...
```
INCOMING → <FISH-handoff> from: explorer · sigil: smaller × known · archetype: nemo
...
### 6.3 Willie RFC — 2 weeks

```
INCOMING → <FISH-handoff> from: explorer · archetype: willie
           artifacts: market scan, interviews (n=8), premortem, nugget board

1. Solidifier reads handoff. Confirms Willie sigil.
2. Solidifier: "This is a Willie. Artifact: PT (pitch/RFC). Expect 1–3 weeks.
...
9. Solidifier runs HO → Builder. Flags that Builder should break this Willie
...
- **Measurement plan is required for Salmon and Willie.** Don't let those archetypes exit Solidify without an MP.
...
### 8.2 Pitch / RFC (PT) template — Willie only
...
- Willie: + per-stage AC (per rollout stage).

### 8.4 Measurement plan (MP) template — Salmon / Willie
...
locked:
  ...
  - "Measurement hooks (Salmon/Willie): {names of events to instrument}"
open:
  - "{non-blocking deferral, if any, with 'defer to v2' tag}"
artifacts:
  - "solidify/{brief-or-pitch}.md"
  - "solidify/{storyboard-or-wireframe}.{ext}"
  - "solidify/{decision-log}.md  (Willie)"
confidence_to_advance: {0.0–1.0}
notes: |
  One shape: {which direction was chosen, one line}.
  Why this over alternatives: {one line — pointer to DL for Willie}.
  Build order recommended: {vertical slices for Tuna/Willie}.
</FISH-handoff>
```
...
- ❌ Leave the brief longer than a page (except Willie pitches, which may be longer but must stay journey-structured).
