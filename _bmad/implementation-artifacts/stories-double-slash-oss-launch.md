# Epics & Stories — Double Slash OSS Launch (Milestone 1)

**PRD:** [`../planning-artifacts/prd-double-slash-oss-launch.md`](../planning-artifacts/prd-double-slash-oss-launch.md) (v1.1)
**UX:** [`../planning-artifacts/ux-design-double-slash-oss-launch.md`](../planning-artifacts/ux-design-double-slash-oss-launch.md)
...
**As a** developer on the Double Slash team,
...
**As a** Senior IC using Double Slash,
**I want** a persistent macOS menubar icon that shows capture state at a glance and opens a quick-info popover,
**So that** I can see Double Slash is working and take quick actions (pause, open) without context-switching.
...
- [ ] AC-004: Clicking the icon opens a 280pt-wide popover showing: one-line status, today's session count per tool, [Pause capture] button, [Open Double Slash] button.
...
**Design reference:** [UX doc — IA diagram + menubar popover](../planning-artifacts/ux-design-double-slash-oss-launch.md#information-architecture)
...
**As a** user opening Double Slash,
...
**Design reference:** [UX doc — IA diagram](../planning-artifacts/ux-design-double-slash-oss-launch.md#information-architecture), [Visual direction](../planning-artifacts/ux-design-double-slash-oss-launch.md#visual-direction-for-shenhav)
...
**As a** user installing Double Slash for the first time,
**I want** a guided, under-5-minute onboarding that gets me from install to first capture,
**So that** I feel in control of what Double Slash does, and I end up with at least one captured session.
...
**Design reference:** [UX doc — Flow 1](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-1--first-run-onboarding-install--first-capture)
**Technical notes:** Welcome copy per v1.1 reframe: *"Welcome to Double Slash. Double Slash carries your AI work between tools — and, when your team is on it, between teammates. Today we're in OSS launch: single-player, local-only. Multiplayer handoff ships in V1."* Don't overclaim V1 features as live.
**Estimated effort:** L
...
**As a** user granting Double Slash Accessibility permission (and as any user navigating the app),
...
**So that** I understand what I'm granting, and I can use Double Slash without a mouse if I need to.

**Acceptance Criteria:**
- [ ] AC-001: Before the macOS Accessibility dialog opens, Double Slash shows a panel explaining exactly what will be read and why, with a single [Open System Settings] action.
- [ ] AC-002: Clicking [Open System Settings] deep-links to the correct pane in System Settings → Privacy & Security → Accessibility.
- [ ] AC-003: If the user denies or dismisses the dialog, Double Slash gracefully disables Tier 3 and surfaces a non-blocking message per REQ-005.
...
**Design reference:** [UX doc — Accessibility](../planning-artifacts/ux-design-double-slash-oss-launch.md#accessibility), [Flow 1 step 5](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-1--first-run-onboarding-install--first-capture)
...
The core reason Double Slash exists. Ship three independent capture paths that together cover every AI tool the beachhead persona uses daily. Each tier is independent — any one failing must not break the others.
...
**I want** to paste my keys into Settings and have Double Slash store them securely,
...
- [ ] AC-004: Screen shows persistent reassurance copy: *"Double Slash never sees these keys leave this device."*
...
**Design reference:** [UX doc — Flow 5 Settings](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-5--settings)
...
**I want** an extension that captures my sessions and sends them to the Double Slash desktop client,
...
- [ ] AC-003: Extension icon shows a "Double Slash is capturing" badge when active on a supported site.
- [ ] AC-004: Extension never transmits captured content to any endpoint other than the local Double Slash desktop client.
...
**Design reference:** [UX doc — Flow 4 Tools & Sources](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-4--tools--sources)
...
- [ ] AC-006: If the buffer fills (>10 sessions pending), the extension notifies the user in the extension popup with an actionable message ("Double Slash desktop client unreachable — open it to catch up").
...
**I want** Double Slash to capture my sessions from those native apps,
...
- [ ] AC-005: End-to-end test: starting a fresh session in Claude Desktop or Cursor produces a session in Double Slash's Today view within 5 seconds of session end.
...
**Design reference:** [UX doc — Flow 4 Tools & Sources](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-4--tools--sources)
**Technical notes:** MCP path: ship a Double Slash MCP server the user registers with each target app. AX path: read the target app's Accessibility tree via macOS Accessibility API (needs explicit permission per Story 1.5). Either path requires the Apple Developer entitlement set up in Story 1.1.
**Estimated effort:** L
...
**Design reference:** [UX doc — error message pattern](../planning-artifacts/ux-design-double-slash-oss-launch.md#copy-system)
...
**Design reference:** [UX doc — Flow 1 step 5, Flow 5 Settings Data & privacy](../planning-artifacts/ux-design-double-slash-oss-launch.md)
...
**So that** I never feel like Double Slash has my data hostage.
...
**Design reference:** [UX doc — Flow 5 Data & privacy, Flow 6 Privacy dashboard](../planning-artifacts/ux-design-double-slash-oss-launch.md)
...
Two in-process agents do the work that makes Double Slash feel alive: tag every session with useful metadata, and flag anything sensitive before it could ever be pushed. Every agent action is logged and narratable.

### Story 4.1 — Agent runtime + audit log

**As a** user of Double Slash,
...
**Design reference:** [UX doc — Flow 3 session detail right rail](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-3--session-detail-view), [Flow 6 Privacy dashboard](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-6--privacy-dashboard)
...
**Design reference:** [UX doc — session card](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-2--seeing-todays-captured-work-today-view)
...
**I want** Double Slash to detect sensitive content in every session and show me what *would* be redacted if pushed,
...
**Design reference:** [UX doc — Flow 3 redaction preview](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-3--session-detail-view)
...
**As a** Senior IC opening Double Slash after a morning of AI work,
...
- [ ] AC-005: Empty state (no sessions ever) shows the v1.1 reframe copy: *"Double Slash is watching {N} tools. Start a session in {Claude/Cursor/ChatGPT} and it'll show up here — ready to hand off to a teammate when V1 ships."* Footer: *"Nothing has left this device."*
...
**Design reference:** [UX doc — Flow 2](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-2--seeing-todays-captured-work-today-view)
...
**Design reference:** [UX doc — Flow 3](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-3--session-detail-view)
...
**Design reference:** [UX doc — Flow 4](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-4--tools--sources)
...
**I want** a single page that answers "what does Double Slash know and what has left this device?",
**So that** I can screenshot it and recommend Double Slash to my team without equivocation.
...
**Design reference:** [UX doc — Flow 6](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-6--privacy-dashboard)
...
**So that** I can configure Double Slash and trust the redaction model.
...
**Design reference:** [UX doc — Flow 5 Settings](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-5--settings), [Flow 3 redaction preview](../planning-artifacts/ux-design-double-slash-oss-launch.md#flow-3--session-detail-view)
...
**As a** Double Slash team member measuring adoption,
...
- [ ] AC-003: Destination is a self-hosted PostHog OSS instance under the Double Slash team's control.
...
**Design reference:** [UX doc — Flow 1 step 3, Flow 5 Telemetry](../planning-artifacts/ux-design-double-slash-oss-launch.md)
...
**As a** Double Slash team member fixing crashes,
...
- [ ] AC-003: Destination is a self-hosted Sentry OSS instance under the Double Slash team's control.
...
**Design reference:** [UX doc — Flow 1 step 3](../planning-artifacts/ux-design-double-slash-oss-launch.md)
...
The non-code work that makes Double Slash a credible OSS launch. If this epic slips, the code could ship but the *launch* can't.

### Story 7.1 — Public GitHub repo + FSL-1.1-Apache-2.0 license + CLA

**As a** prospective contributor,
**I want** to land on the Double Slash repo and immediately understand the license, contribution path, and governance,
**So that** I don't waste my time or fear being sued.

**Acceptance Criteria:**
- [ ] AC-001: Two public repos exist in the Double Slash GitHub org: one for the client+extension (FSL-1.1-Apache-2.0), one for the capture protocol (Apache 2.0).
...
**As a** security-conscious user auditing Double Slash's supply chain,
**I want** every release to be signed and accompanied by an SBOM with no license landmines,
**So that** I can verify provenance and recommend Double Slash to my team with confidence.
...
### Story 7.3 — Marketing site at `doubleslash.<domain>`

**As a** visitor arriving from Hacker News or Twitter,
**I want** a fast, honest landing page that explains what Double Slash does and lets me install it in under a minute,
**So that** my first impression is calm confidence, not marketing noise.

**Acceptance Criteria:**
- [ ] AC-001: Site at `doubleslash.<domain>` (exact domain TBD) ships with: hero + tagline + 20s demo loop (menubar + Today view), Privacy-dashboard screenshot as the centerpiece, "how it works" tier diagram, founder story (Tal + Shenhav), install + GitHub + license links.
- [ ] AC-002: Site does NOT include testimonials, enterprise logos, or customer quotes.
- [ ] AC-003: Site hero states the team-collab framing from v1.1: Double Slash is a collaboration tool for design+dev teams, not a personal AI brain.
- [ ] AC-004: Site is built on Next.js + Vercel and open-sourced in the Double Slash GitHub org under the same FSL license.
...
**Design reference:** [UX doc — Marketing site section](../planning-artifacts/ux-design-double-slash-oss-launch.md#marketing-site-v0--not-part-of-the-app-but-gated-together-at-launch)
