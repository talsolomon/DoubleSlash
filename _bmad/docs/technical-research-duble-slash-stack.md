# Technical Research — Duble Slash Stack & Feasibility

**Date:** 2026-04-20
**Question:** What's the realistic technology stack for Duble Slash's OSS launch (6 weeks) and v1 multiplayer (3 months later) given a two-designer founding team and the integrity-first product profile? Where are the swing risks?

> **Stack note:** BMAD's default assumption was a React+Vite web app. Duble Slash is a **desktop capture client + browser extension + (later) hosted backend + agent fabric** — fundamentally different. This report establishes the real stack from scratch.
...
| **Capture — Tier 0 (API key)** | Local proxy / pass-through SDK wrappers | User points their app at `http://localhost:port`; Duble Slash forwards to OpenAI/Anthropic and records. Zero ToS risk. |
...
**How it works:** User installs Duble Slash, enters their OpenAI / Anthropic / etc. API key. Duble Slash runs a local HTTP proxy on `localhost:<port>`. User configures their tools to point at the local proxy (or installs a Duble Slash-aware SDK wrapper). Duble Slash forwards every call to the real provider, records the request + response, returns to the caller.
...
**How it works:** macOS exposes window content to apps the user grants Accessibility permission to (System Settings → Privacy → Accessibility). Duble Slash queries the AX tree of Claude Desktop, Cursor, etc. and reads the rendered conversation as it changes.
...
The `//` gesture (consent-per-session capture trigger) is not a capture *mechanism* — it's an invocation primitive the user types inside the target AI tool. Making the target tool *recognize* `//` as "hand this session to Duble Slash" requires **tool-specific install plumbing**:
...
- Run on the user's own API key (BYOK) so the user pays inference and Duble Slash doesn't carry the cost.
...
- BYOK on personal agents = Duble Slash doesn't pay for inference at the seat where 90% of inference happens.

**Reject local-LLM-only agent runtime (Ollama, llama.cpp).** Local model quality in 2026 is good but not at Sonnet/Haiku level for the agent tasks Duble Slash needs (structured tool use, redaction precision). Ship hosted-LLM with BYOK; revisit local models in v2.
...
| Trademark for "Duble Slash" not clearable | Medium | Medium (delays public launch) | Start clearance search this week. |
...
Personal-agent inference is BYOK so doesn't hit Duble Slash's bill.
