...
- **`AGENTS.md` + `CLAUDE.md` instruction files are now standard.** Copilot, Cursor, and Claude Code all auto-load them. This is the install-bundle surface for FISH — our `//explore` / `//solidify` / `//build` / `//ship` commands should be distributed as entries in these files, not as plugins.
...
- **Where we differ:** We ship a **methodology** (FISH) with opinionated phase gates, archetype classification, and a handoff contract. Claude Code gives you a toolbox; we give you a kitchen.
- **Where we're at risk:** The `skills` architecture is designed exactly for what we want to ship. If someone packages FISH-like methodology as a Claude Code skill bundle before we do, we are distributing a subset of that ecosystem, not a category.
...
- **Where we're at risk:** They have a much larger install footprint and their phase agents almost exactly mirror ours. If GitHub adds methodology enforcement, they are FISH-for-GitHub-only — which for many teams is enough.
...
- **Where we differ:** Devin replaces the engineer; we augment the designer + engineer + PM with a shared method. Devin is a job title; FISH is a collaboration grammar.
...
- **Where we differ:** We are a methodology; LangGraph is the graph engine you'd implement our methodology on. Our `<FISH-handoff>` contract could be a LangGraph state-transition schema.
...
- **Where we differ:** CrewAI gives you the persona primitive; FISH gives you the phase structure + axis modulation + handoff discipline *around* personas.
...
- **Where we differ:** We enforce discrete handoffs instead of open conversation. FISH explicitly rejects "let the agents talk it out."
- **Risk:** Zero direct.
...
- **Where we differ:**
  1. **FISH is designer/PM-led; BMAD is engineer-led.** BMAD optimizes for "requirements → sprint → PR." FISH optimizes for "idea → validated brief → trustable implementation" with the designer as the persistent voice.
  2. **FISH ships a cross-tool capture + redaction + handoff layer (Tally, Cipher, Relay, Beacon, Pack, Echo, Twin, Gate, Loom).** BMAD lives inside the IDE. We live *between* tools.
  3. **FISH has axis modulation (size × certainty) + archetypes (Nemo/Tuna/Salmon/Willie).** BMAD has workflow templates per task type but no unified axis system.
  4. **FISH defines an explicit `<FISH-handoff>` contract** with trust receipts and reverse-flow semantics. BMAD's handoffs are implicit in workflow file ordering.
- **Where we're at risk:**
...
| **Duble Slash (FISH)** | **C** | **4 local + 9 system** | **Persona (13)** | **Yes (full + axes + archetypes + handoffs)** | **Local + desktop client** | **Yes (planned)** | **TBD (freemium)** |
...
2. **Designer-led, not engineer-led.** BMAD, Devin, Cursor, Claude Code are all engineer-centric. Copilot Workspaces is GitHub-centric. **The designer / PM / founder who uses Claude Desktop + Figma + a notes tool has no dedicated methodology.** FISH is that.
3. **Axis modulation as first-class.** The `size × certainty` dial controlling persona behavior is unique. Nobody else modulates agent *posture* by card archetype. This is the FISH article's most novel contribution.
...
5. **Persona-bearing methodology for non-code work.** FISH works for research, brand, strategy, design — not just code. BMAD is code-first. Devin is code-only. **The "design sprint methodology, but with AI agents" space is unclaimed.**
...
3. **Someone packages FISH-like methodology as a Claude Code skill bundle.** The `skills` architecture + community skill libraries (e.g. `wshobson/agents`) are exactly the right surface. A single well-written MIT-licensed skill bundle could preempt the OSS drop.
...
3. **Treat BMAD as the incumbent to differentiate *against*.** Our tagline, our docs, our install experience should all answer: "Why FISH instead of BMAD?" Three reasons to memorize: (a) designer-led not engineer-led, (b) cross-tool not IDE-locked, (c) axis modulation + archetypes + reverse-flow handoffs. Ship a comparison page at launch.
